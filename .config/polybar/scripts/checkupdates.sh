...
if (( $UPDATES_COUNT > 0 )); then
    notify-send -i $NOTIFY_ICON '<span color="#e2e2e2">'$UPDATES_COUNT' updates available</span>'
    ...
    if ! [ $OnLine ]; then
        notify-send -i $NOTIFY_ICON '<span color="#e2e2e2">Cannot check for updates (no network connection)</span>'
        echo "   %{F#728cbb T3}%{F#e2e2e2 O-4 T5}%{O0 T1}   "
    else
        notify-send -i $NOTIFY_ICON '<span color="#e2e2e2">System is up to date</span>'
        echo "   %{F#728cbb T3}%{T1}   "
    fi
fi
